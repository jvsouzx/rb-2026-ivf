...
A API foi implementada em C++20 usando `uWebSockets`, com duas instâncias atrás do Nginx. O payload recebido no endpoint é transformado em um vetor normalizado de 14 dimensões, concentrando variáveis numéricas, categóricas e booleanas em uma representação única para permitir comparação por distância.
...
No endpoint `/fraud-score`, a resposta JSON é montada manualmente para reduzir overhead de serialização. O Nginx também foi alterado de proxy HTTP/L7 para `stream` TCP/L4, reduzindo o overhead de balanceamento na frente das APIs.

### Build

O build foi ajustado para performance com `-O3`, `-march=haswell`, `-mtune=haswell`, `-mavx2`, `-funroll-loops` e LTO. O alvo `haswell` foi escolhido porque a arquitetura da máquina do teste final era conhecida, então fazia sentido otimizar para esse perfil.

<!-- Trade-offs -->
## Trade-offs

Algumas decisões foram tomadas pensando nas restrições do desafio, principalmente CPU, memória e latência.

- A escolha por IVF-Flat troca parte da exatidão do kNN exato por performance. Em vez de comparar contra todas as referências, a busca consulta apenas as listas invertidas mais próximas, reduzindo bastante o número de candidatos avaliados em runtime.
- A quantização dos vetores de `float` para `int16_t` reduz um pouco a precisão dos valores, mas diminui o uso de memória e deixa o cálculo de distância mais barato.
- O pré-processamento no build deixa a imagem mais acoplada ao dataset gerado, mas remove do startup e do caminho crítico o custo de ler, descomprimir e converter o `references.json.gz`.
- O carregamento via `mmap`, combinado com `madvise` e `warmReferences()`, melhora o uso de memória e reduz latências frias nas primeiras buscas, mas aumenta o trabalho feito antes da API responder `/ready`.
- Os parâmetros `nprobe` e `candidate_cap` permitem controlar o custo máximo da busca. Valores menores reduzem latência e CPU, enquanto valores maiores aumentam a chance de encontrar candidatos melhores.
- As otimizações com AVX2 e alvo `haswell` aceleram o cálculo de distância aproveitando a arquitetura da máquina usada no teste final. O custo é reduzir a portabilidade para CPUs fora desse perfil.
- A troca do Nginx de proxy HTTP/L7 para proxy TCP/L4 reduz overhead no balanceamento, mas abre mão de recursos de camada HTTP, como inspeção de payload, roteamento por path e métricas mais detalhadas no load balancer.
...
- Docker
- Docker Compose
- Acesso à internet no primeiro build, porque a imagem instala dependências e prepara o ambiente de build
- CPU `x86_64` compatível com Haswell/AVX2 para reproduzir o mesmo perfil usado na submissão
...
A API pública fica disponível em `http://localhost:9999`. O Nginx recebe as conexões nessa porta e encaminha para as instâncias da API na porta interna `8080`.

Verificando prontidão:
...
Exemplo de chamada para `/fraud-score`:
...
Além dos testes manuais com `curl`, a solução foi testada localmente usando o teste de carga oficial da Rinha. O fluxo usado foi subir este `docker-compose` e, em seguida, executar o k6 a partir do repositório oficial.

Neste ambiente, o repositório oficial estava no diretório ao lado:

```text
../rinha-de-backend-2026
```

Com a API rodando em `localhost:9999`, o smoke test pode ser executado com:

```shell
docker compose -f ../rinha-de-backend-2026/test/docker-compose.yml --profile smoke up --abort-on-container-exit k6-smoke
```

O teste de carga completo pode ser executado com:

```shell
docker compose -f ../rinha-de-backend-2026/test/docker-compose.yml --profile test up --abort-on-container-exit k6
```

O compose oficial do teste usa `network_mode: host`, então o k6 acessa diretamente a API exposta por este projeto em `http://localhost:9999`.
